...
from .utils import clean_str, safe_unicode, have_common_element
...
class Genius(API, PublicAPI):
    ...
    def __init__(
        ...
        self.songs_only_in_english = songs_only_in_english
    ...
    def song_tags(self, song_id=None, song_url=None):
        """Gets the tags of a song.

        Args:
            song_id (:obj:`int`, optional): Genius song ID
            song_url (:obj:`str`, optional): Genius song URL
        ...
        html = BeautifulSoup(self._make_request(path, web=True), "html.parser")

        # Determine the class of the div
        divs = html.find_all("a", class_=re.compile("^tags$|SongTags__Tag"))
        ...
        tags = [div.get_text() for div in divs]
    ...
    def _result_is_lyrics(self, song):
        ...
        Args:
            ...
            song_tags (:obj:`list`, optional): List of tags for the song.
        ...
        # Check if the song has any of the excluded terms in the title
        expression = r"".join(["({})|".format(term) for term in self.excluded_terms])
        expression = expression.strip("|")
        regex = re.compile(expression, re.IGNORECASE)
        if regex.search(clean_str(song["title"])):
            if self.verbose:
                print("Specified song has excluded terms in title. Rejecting.")
            return False

        tags = self.song_tags(song_url=song["url"])
        # Check if the song is in English
        if self.songs_only_in_english and "In English" not in tags:
            if self.verbose:
                print("Specified song is a non-English song. Rejecting.")
            return False

        # Check if the song is tagged as non-music
        if have_common_element(tags, self.non_music_tags):
            if self.verbose:
                print("Specified song is a non-music song. Rejecting.")
            return False
    ...
    def search_album(
        self, name=None, artist="", album_id=None, get_full_info=True, text_format=None
    ):
        ...
        if album_id:
            ...
            album_info = self._get_item_from_search_response(
                response, name, type_="album", result_type="name"
            )
    ...
    def search_song(self, title=None, artist="", song_id=None, get_full_info=True):
        ...
        if song_id:
            ...
            result = self._get_item_from_search_response(
        ...
        while not reached_max_songs:
            ...
            for song_info in songs_on_page["songs"]:
                ...
                if not valid:
                    if self.verbose:
                        s = song_info["title"]
                        print('"{s}" is not valid. Skipping.'.format(s=safe_unicode(s)))
                    continue
                ...
                if reached_max_songs:
                    if self.verbose:
                        print(
                            (
                                "\nReached user-specified song limit ({m}).".format(
                                    m=max_songs
                                )
                            )
                        )
    ...
    def save_artists(
        self, artists, filename="artist_lyrics", overwrite=False, ensure_ascii=True
    ):
